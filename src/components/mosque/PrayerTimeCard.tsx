...
export const PrayerTimeCard: React.FC<PrayerTimeCardProps> = ({
  ...
  return (
    <div
      className={cn(
        'relative flex flex-col items-center justify-center p-6 rounded-2xl transition-all duration-500',
    ...
    >
      ...
      {isNext && (
        <div className="absolute inset-0 rounded-2xl bg-primary/10 animate-glow" />
      )}
      
      <h3 
        className={cn(
          'font-arabic text-2xl md:text-3xl mb-2 transition-colors',
          isNext && 'text-primary',
      ...
      <p 
        className={cn(
          'font-display text-3xl md:text-4xl font-bold tabular-nums',
          isNext && 'text-primary',
          isCurrent && 'text-secondary',
      ...
      {isNext && (
        <span className="mt-2 text-sm text-primary/80 font-display">
          الصلاة القادمة
        </span>
      )}
    </div>
